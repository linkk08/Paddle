...
void AdaptiveLayerNormXPUKernel(const Context& ctx,
  ...
  if (tensor1_data) {
    ...
    std::vector<int> tensor1_shape_vec = common::vectorize<int>(tensor1.dims());
    DenseTensor tensor1_fp32;
    ...
    r = baidu::xpu::api::broadcast_mul(xpu_ctx->x_context(),
                                       ...
                                       tensor1_fp32.data<float>(),
                                       out_data,     // 原址输出
                                       x_shape_vec,  // layer_norm输出的维度
                                       tensor1_shape_vec);  // scale输出的维度
    PADDLE_ENFORCE_XDNN_SUCCESS(r, "broadcast_mul");
  }
  if (tensor2_data) {
    tensor2 = phi::funcs::Unsqueeze(tensor2, unsqueeze_axis2);
    std::vector<int> tensor2_shape_vec = common::vectorize<int>(tensor2.dims());
    DenseTensor tensor2_fp32;
    if (tensor2_data.get().dtype() == phi::DataType::FLOAT16 ||
